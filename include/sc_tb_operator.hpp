...
class SCTBOp
{
	...
	SCTBOp(const kpm::integer _dim, const kpm::integer _max_coord):
...
//		#pragma omp parallel for firstprivate(scal) private(row,idx)
	...
	char mkl_matdescra[7];
	int matdescra[7];

	void MatDescraInit()
	{
		mkl_matdescra[0]='G';	matdescra[1]=0; //general=0,G; her=2,H
		mkl_matdescra[1]='L';	matdescra[2]=1;  //lower=1,L; upper=2,U
		mkl_matdescra[2]='N';	matdescra[3]=0; //empty diagonal
		mkl_matdescra[3]='C';	matdescra[4]=0;
		mkl_matdescra[4]=0;	matdescra[5]=0;	
		mkl_matdescra[5]=0;	matdescra[6]=0;	
	}
};
