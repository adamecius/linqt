...
template <typename Dim,typename Matrix>	void lattice(Dim Nx, Dim Ny, Matrix& H)
	{
		...
		Scalar E0=(double) 0;
		Scalar t1=(double)-1;
		std::cout<<"Nx="<<Nx<<" Ny="<<Ny<<" v="<<v<<std::endl;
		for(Integer i=0;i<Nx;i++)for(Integer j=0;j<Ny;j++){
			//We start Filling the hoppings of the Matrix
	
	//ZERO NEIGHBOR
			/*Onsite (i,j)-->(i,j)*/
			k=((i+0+Nx)%Nx)*Ny+(j+0+Ny)%Ny;								
			h=0;	
			H.row_indices[h+k*v]	= k;								//Site: (i  ,j  )B	<----> h=0
			H.column_indices[h+k*v]	= k;								//Site: (i  ,j  )B	<----> h=0
			H.values[h+k*v]		= 0;
	//FIRST NEIGHBOR
			/*Hoping (i,j)<-->(i-1,j+0)*/						
			q=((i-1+Nx)%Nx)*Ny+(j+0+Ny)%Ny;								
			h=1;
			H.row_indices[h+k*v]	= k;								//Site: (i  ,j  )A					
			H.column_indices[h+k*v]	= q;								//Site: (i  ,j  )B
			H.values[h+k*v]			= t1;
			q=((i+1+Nx)%Nx)*Ny+(j+0+Ny)%Ny;								
			h=2;
			H.row_indices[h+k*v]	= k;								//Site: (i  ,j  )A					
