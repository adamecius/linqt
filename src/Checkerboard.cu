	...
	#include "checkerboard_lattice_cusp.h"
	...
	int main(int argc, char *argv[])    			//Para simplificar las corridas elegimos N=argv[1], M=argv[2]
	{
		...
		int Nx, Ny,DD,M, R,ngpu,nflux,B;	//variables de conteo
		...
		if(argc>=9+1){
			...
			std::cout<<"Nx, Ny, M, U,IndexFlux ,R  ,MachineName,GPU, seed"<<std::endl;
			return 0;}
		srand(SEED);
		...
		cudaSetDevice(ngpu);
		DD	=	2*Nx*Ny;
		HCOO H (DD,DD,13*DD); // 13 vecinos= 1zero+ 4primeros+ 4segundo+ 4terceros
		HCOO Vx(DD,DD,13*DD); // 13 vecinos= 1zero+ 4primeros+ 4segundo+ 4terceros
		HCOO Vy(DD,DD,13*DD); // 13 vecinos= 1zero+ 4primeros+ 4segundo+ 4terceros	

		square::lattice(Nx,Ny,H);
		square::Anderson(Nx,Ny,H,U);
//		square::Magnetic_Field(Nx,Ny,H,nflux);
		square::velocityx(Nx,Ny,H,Vx);
//		square::velocityy(Nx,Ny,H,Vy);
		RefineSparse(H);
//		RefineSparse(Vx);
//		RefineSparse(Vy);
		Emin=-5.0;
