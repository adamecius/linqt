...
{	
	...
	for( int i=0; i < num_div; i++){
	  energies[i] = cos( M_PI * ( 2 * i + 0.5 )/num_div );//-xbound + i*(2*xbound)/(num_div-1) ;
	}
	...
	std::string
	outputName  ="mean"+mu.SystemLabel()+"_noKernel_FFTgrid_yesDisp_begendMEAN_abs.dat";
