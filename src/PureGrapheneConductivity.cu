...
int main(int argc, char *argv[])    			//Para simplificar las corridas elegimos N=argv[1], M=argv[2]
{
	...
	chebyshev::random::DOS(Nx,Ny,H,M,R,Emin,Emax,alpha,dospath,65536+1);
	cycletime(-1);
	std::cout<<"Calculating SIGMAXX and SIGMAXY BASTIN"<<std::endl;
	chebyshev::random::SIGMA(Nx,Ny,H,Vx,Vy,M,R,Emin,Emax,alpha,sigmaxxpath,sigmaxypath,65536+1);
