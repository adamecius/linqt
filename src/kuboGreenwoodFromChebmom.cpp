...
double diff_fermi_dirac( const double x )
{
	if ( std::fabs(x) > 40.0)
		return 0.0;
...
{	
	...
	if( eta_phi > tbr)
		min_res=tbr;
	const double max_inn_sum = (int)( 4.0/(min_res/HalfWidth/1000.) ); 
	...
	std::string
	outputName  ="NEQ_FL"+S_OPR+LABEL+"KPM_M"+to_string(maxNumMom)+"x"+to_string(maxNumTMom)+"RS"+to_string(numRV)+"tphi"+(std::string)tphi+"tmax"+(std::string)tmax+".dat";
	...
	for( double E=Emin; E < Emax; E+=dE)	
	{
		...
		{
			//The chebyshev node 
			const double _xn = cos( M_PI*( n + 0.5 )/(double)max_inn_sum); 
			if( std::fabs(_xn) <= xbound )
			{
				//The chebyshev node 
				const double _xn = cos( M_PI*( n + 0.5 )/(double)max_inn_sum); 
				//The Fermi dirac distribution used solely normalized  quantities  
//				double dfdp = diff_fermi_dirac( ( nE-_xn)/ntb);
//				double dfdn = diff_fermi_dirac( (-nE-_xn)/ntb);
//				double _dfn =-( dfdp + dfdn )*0.5/ntb;
				double _dfn =-diff_fermi_dirac( ( nE-_xn)/ntb)/ntb;
				//Then one compute the integrated  bessel coefficients for a particular chebyshev node
//				besselCoeff::BesselBastinArr(ntphi,0.0,ntmax,_xn,maxNumTMom,&BesselCn_[0] );
					...
					#pragma omp parallel 
					{
						...
						{ 
							...
							for( int mt = 0 ; mt < maxNumTMom ; mt++)
							{
								...
								const double  C_m0 = g_m0*M_PI*cos( m0*(M_PI*( n + 0.5 )/(double)max_inn_sum ) )/max_inn_sum;				
								const complex C_nt = gn_t*besselCoeff::GCoeff( mt, xn );
								my_neq+= dfn*C_m0*(C_nt*mom0).imag();
							}
							neq_[m0] += my_neq;
						}
					}
		...
		const double scale_factor = dim/HalfWidth/HalfWidth/M_PI/2.0;//two is due to kubo greenwood
