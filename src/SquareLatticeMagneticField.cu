...
int main(int argc, char *argv[])    			//Para simplificar las corridas elegimos N=argv[1], M=argv[2]
{
	...
	std::cout<<"Setting device "<<ngpu<<std::endl;
   	cudaSetDevice(ngpu);
	DD	=	Nx*Ny;
	HCOO H (DD,DD,5*DD); // 4 vecinos=1zero+ 4primeros
	HCOO Vx(DD,DD,5*DD); // 4 vecinos=1zero+ 4primeros
	HCOO Vy(DD,DD,5*DD); // 4 vecinos=1zero+ 4primeros	
