...
{	
	...
	std::string
	outputName  ="NEQ"+S_OPR+LABEL+"KPM_M"+to_string(maxNumMom)+"x"+to_string(maxNumTMom)+"RS"+to_string(numRV)+"tphi"+(std::string)tphi+"tmax"+(std::string)tmax+"kernel.dat";
	...
	const double max_inn_sum = maxNumMom; 
	double  neq = 0.0;
	for( double n=0.0; n < max_inn_sum; n=n+1.0 )
	{
		...
		//					g_m0 = JacksonKernel(maxNumMom,m0);
						...
						double gn_t = sinh(lambda_mt*( 1.0 - (double)mt/maxNumTMom))/sinh(lambda_mt) ;
						const complex C_nt= gn_t*besselCoeff::dGCoeff( mt, xn ); 
						my_neq+= C_m0*(C_nt*mom0).imag();
					}
				}
				_my_neq += my_neq;
			}
			...
			const double scale_factor = dim/HalfWidth/HalfWidth/M_PI;
			outputfile<<HalfWidth*_xn<<" "<<neq*scale_factor<<" "<<std::endl;
		}
	}
