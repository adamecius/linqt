...
int main(int argc, char *argv[])    			//Para simplificar las corridas elegimos N=argv[1], M=argv[2]
{
	...
	if(argc>=12+1){
        ...
        std::cout<<"Nx, Ny, M, W, ISOa,p , R,Lx, Ly  ,MachineName,GPU"<<std::endl;
        return 0;}
	srand(SEED);
	...
	dataID+="ISOa";		dataID+=argv[5];
	dataID+="p";		dataID+=argv[6];
	dataID+="R";		dataID+=argv[7];
	dataID+="Lx";		dataID+=argv[8];
	dataID+="Ly";		dataID+=argv[9];
	dataID+=argv[10] ;
	...
	std::string spec_sigmaxx_path("data/GrapheneHaldaneSpectralConductivityXX");
	spec_sigmaxx_path.append(dataID);
	spec_sigmaxx_path+=".dat";

	//Creates the file that will hold the Longitudinal Conductivity
	std::string spec_sigmaxy_path("data/GrapheneHaldaneSpectralConductivityXY");
	...
	//Creates the file that will hold the Density of states 
	std::string dosmappath("data/GrapheneHaldaneDOSMAP");
	dosmappath.append(dataID);
	dosmappath+=".dat";
	//Creates the file that will hold the Longitudinal Conductivity
	std::string sigmaxxmappath("data/GrapheneHaldaneConductivityXXMAP");
	...
	chebyshev::realspace::LinearDosConductivity(Nx,Ny,H,Vx,M,Emin,Emax,alpha,dosmappath,sigmaxxmappath,(FloatType)0.0,sqrt(3)*(Lx+0.5*Ly),1.5*Ly);
	cycletime(-1);
	cycletime(-1);
