...
int main(int argc, char *argv[])
{
	...
	//OUTPUT VARIABLE
	std::ofstream outputfile( ("NonEqOp"+S_OPR+LABEL+"KPM_M"+sNumMom+"E0"+sE0+"RVConv.dat").c_str() );
	srand(1521);
	const
	double 
	...
	for( int r = 0 ; r < numRV ; r++) 
	{
		...
		outputfile<<r<<" "<< output_quantity*4.0/bandWidth/bandWidth/(double)(r+1)/(double)dim<<std::endl;
		std::cout<<r<<" "<< output_quantity*4.0/bandWidth/bandWidth/(double)(r+1)/(double)dim<<std::endl;
	}
	outputfile.close();
	delete [] data;
	return 0;
