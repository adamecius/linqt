   ...
   const int
    ...
    //while (stateFactory.CreateNewState())
        ...
        for (int m0 = 0; m0 < cTable.Size_InDir(0); m0 += batchSize)
        {
 		...
 		Jswap = JL[batchSize-2];  JL[batchSize-2]=JL[0]; JL[0]=Jswap;
 		Jswap = JL[batchSize-1];  JL[batchSize-1]=JL[1]; JL[1]=Jswap;
            }
 		Jswap = JR[batchSize-2];  JR[batchSize-2]=JR[0]; JR[0]=Jswap;
 		Jswap = JR[batchSize-1];  JR[batchSize-1]=JR[1]; JR[1]=Jswap;
	}
    }
    delete[] JL;
    delete[] JR;
