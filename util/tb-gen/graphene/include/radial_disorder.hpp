...
{		
	...
	std::cout<<L0<<" "<<myDis.range<<std::endl;;
	...
	const Real CellCutOff = range_tol*myDis.range ; 
	...
	if(  (double)rand()/(double)RAND_MAX < myDis.concen|| (ip0== n0/2 && ip1== n1/2) ) //Select a center
//	if(  (double)rand()/(double)RAND_MAX < myDis.concen) //Select a center
	{
		const double 
		Umax = myDis.maxHeight(ip0,ip1,ipo,0);
		...
		for(int IO=0; IO < norb ; IO++ )
		{
			...
			const double
			rdiff[2] ={ (ip0-I0)*A[0][0] + (ip1-I1)*A[1][0]+ (ipo-IO)*Delta[0],
						(ip0-I0)*A[0][1] + (ip1-I1)*A[1][1]+ (ipo-IO)*Delta[1]
						},
			...
			if( dist < CellCutOff*0.5 )
			{
				...
				for( int IS=0; IS<nspin; IS++)
				{
					...
					const int
					k0= radialDisorder::IndexesToIndex(	I0, n0,
												I1, n1,
												IO, norb,
												IS, nspin );
					myDis.PotProfile( I0,I1,IO,IS,Umax,dist,diagElements[k0]);
				}
			}
		}
		imp_num=imp_num+1;
	}
	std::cout<<"The total number of impurities is:"<<imp_num;
 return 0;
 }
...
class PuddleDisorder
{
	...
	void PotProfile(const int i0, const int i1, 
					const int io, const int is,
					Real Umax,Real dist,Real& out)
	{
		out+= Umax*exp( -0.5*dist*dist/range/range );
	}
...
class StaggeredDiskDisorder
{
	...
	Real maxHeight( const int i0, const int i1, 
					const int io, const int is) const
	{
		return height;
	}
